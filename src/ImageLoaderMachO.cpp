...
/**
 ImageLoader是抽象类，其子类负责把Mach-O文件实例化为image，
 当sniffLoadCommands()解析完以后，根据compressed的值来决定调用哪个子类进行实例化
				...
				sniffLoadCommands
                     |
				compress—— NO ——ImageLoaderMachOClassic::instantiateMainExecutable
		             | YES
...
ImageLoader* ImageLoaderMachO::instantiateMainExecutable(const macho_header* mh, uintptr_t slide, const char* path, const LinkContext& context)
{
	...
	/**
	调用ImageLoaderMachO:sniffLoadCommands()函数来获取一些数据，包括：
	 
	  1.compressed：若Mach-O存在LC_DYLD_INFO和LC_DYLD_INFO_ONLY加载命令，则说明是压缩类型的Mach-O
	  2.segCount：根据 LC_SEGMENT_COMMAND 加载命令来统计段数量，这里抛出的错误日志也说明了段的数量是不能超过255个
	  3.libCount：根据 LC_LOAD_DYLIB、LC_LOAD_WEAK_DYLIB、LC_REEXPORT_DYLIB、LC_LOAD_UPWARD_DYLIB 这几个加载命令来统计库的数量，库的数量不能超过4095个。
	  4.codeSigCmd：通过解析LC_CODE_SIGNATURE来获取代码签名加载命令
	  5.encryptCmd：通过LC_ENCRYPTION_INFO和LC_ENCRYPTION_INFO_64来获取段的加密信息
	 */
	sniffLoadCommands(mh, path, false, &compressed, &segCount, &libCount, context, &codeSigCmd, &encryptCmd);
	
	// instantiate concrete class based on content of load commands
	
	if ( compressed ) /// 根据compressed的值来决定调用哪个子类进行实例化
...
/**
 接下来循环调用每个镜像的registerInterposing()函数，该函数会遍历Mach-O的LC_SEGMENT_COMMAND加载命令，读取__DATA,__interpose，
	并将读取到的信息保存到fgInterposingTuples中，
 接着调用applyInterposing()函数，内部经由doInterpose()虚函数进行替换操作，以ImageLoaderMachOCompressed::doInterpose()函数的实现为例：该函数内部调用了eachBind()与eachLazyBind()，具体处理函数是interposeAt()，该函数调用interposedAddress()在fgInterposingTuples中查找需要替换的符号地址，进行最终的符号地址替换
 */
void ImageLoaderMachO::registerInterposing(const LinkContext& context)
{
	// mach-o files advertise interposing (插入) by having a __DATA __interpose section
	...
	// 遍历Mach-O的LC_SEGMENT_COMMAND加载命令
	for (uint32_t i = 0; i < cmd_count; ++i) {
		switch (cmd->cmd) {
			case LC_SEGMENT_COMMAND:
				{
					...
					for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
						...
						// 读取__DATA,__interpose,并将读取到的信息保存到fgInterposingTuples中
				...
				for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
					...
					if ( type == S_MOD_INIT_FUNC_POINTERS ) {
						...
						for (size_t j=0; j < count; ++j) {
							...
							if ( ! dyld::gProcessInfo->libSystemInitialized ) {
								// <rdar://problem/17973316> libSystem initializer must run first
								/// libSystem initializer must run first  系统库函数libSystem必须先跑起来
...
bool ImageLoaderMachO::doInitialization(const LinkContext& context)
{
	...
	// 调用doImageInit来执行镜像的初始化函数，也就是LC_ROUTINES_COMMAND中记录的函数
	doImageInit(context);
	
	
	// doModInitFunctions()方法来解析并执行_DATA_,__mod_init_func这个section中保存的函数
	// 会调用 c++ 的构造方法
