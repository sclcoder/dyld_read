...
void ImageLoader::link(const LinkContext& context, bool forceLazysBound, bool preflightOnly, bool neverUnload, const RPathChain& loaderRPaths, const char* imagePath)
{
	...
	__block uint64_t t2, t3, t4, t5;
	{
		...
		// 递归进行rebase ，必须对主程序和依赖库做重定位rebase(由于ASLR的存在)
...
/**
 内部再依次调用processInitializers()、recursiveInitialization()。我们在recursiveInitialization()函数里找到了notifySingle()函数
 */
...
void ImageLoader::weakBind(const LinkContext& context)
{
	...
	//	getCoalescedImages()合并所有动态库的弱符号到一个列表里
										  ...
										  InitializerTimingList& timingInfo, UninitedUpwards& uninitUps)
{
	...
	if ( fState < dyld_image_state_dependents_initialized-1 ) {
		...
		try {
			...
			/// 重点: 最终会调用objc库的注册回调
			/// 该函数notifySingle : 该函数找不到load_images的影子，需要详细分析
