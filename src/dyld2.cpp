...
static void notifySingle(dyld_image_states state, const ImageLoader* image, ImageLoader::InitializerTimingList* timingInfo)
{
	...
	if ( (state == dyld_image_state_dependents_initialized) && (sNotifyObjCInit != NULL) && image->notifyObjC() ) {
		...
		// 我们关心的只有sNotifyObjCInit这个回调，继续寻找其赋值的地方为 registerObjCNotifiers函数
		
		/**
		 这是个回调函数的调用 , sNotifyObjCInit 上面判断了并不会为空 , 那就代表一定是有值的 .
		 那我们搜索一下 sNotifyObjCInit , 看看什么时候被赋的值.
		 */
...
/**
	这一步由initializeMainExecutable()完成。dyld会优先初始化动态库，然后初始化主程序。
	该函数首先执行runInitializers()，内部再依次调用processInitializers()、recursiveInitialization()。
	我们在recursiveInitialization()函数里找到了notifySingle()函数：
 */
...
static ImageLoaderMachO* instantiateFromLoadedImage(const macho_header* mh, uintptr_t slide, const char* path)
{
	...
	if ( isCompatibleMachO((const uint8_t*)mh, path) ) {
		/**
		 ImageLoaderMachO::instantiateMainExecutable()函数里面首先会调用sniffLoadCommands()函数来获取一些数据，
		 包括：
		 compressed：若Mach-O存在LC_DYLD_INFO和LC_DYLD_INFO_ONLY加载命令，则说明是压缩类型的Mach-O，代
		 */
		ImageLoader* image = ImageLoaderMachO::instantiateMainExecutable(mh, slide, path, gLinkContext);
		
		
		/**
		 在调用完ImageLoaderMachO::instantiateMainExecutable()后继续调用addImage()，
...
/**
	...
	DYLD_ROOT_PATH---LD_LIBRARY_PATH---DYLD_FRAMEWORK_PATH_DYLD_LIBRARY_PATH---raw path ---DYLD_FALLBACK_LIBRARY_PATH
	
	 当内部调用到loadPhase5load()函数的时候，会先在共享缓存中搜寻，
	 如果存在则使用ImageLoaderMachO::instantiateFromCache()来实例化ImageLoader，
	 否则通过loadPhase5open()打开文件并读取数据到内存后，再调用loadPhase6()，
	 通过ImageLoaderMachO::instantiateFromFile()实例化ImageLoader，
	 最后调用checkandAddImage()验证镜像并将其加入到全局镜像列表中。
 */
...
static void mapSharedCache()
{
	...
	/// mapSharedCache()里面实则是调用了loadDyldCache()
...
void registerObjCNotifiers(_dyld_objc_notify_mapped mapped, _dyld_objc_notify_init init, _dyld_objc_notify_unmapped unmapped)
{
	...
	/**
		这个函数调用 , 其第二个参数赋值给了 sNotifyObjCInit , 然后在 notifySingle 里被执行 .
	 */
	sNotifyObjCInit		= init; /// 给sNotifyObjCInit这个回调赋值
...
static void loadInsertedDylib(const char* path)
{
	...
	try {
		...
		/// LoadContext参数后调用load()函数
		/**
		 load()函数的实现为一系列的loadPhase*()函数，
		 loadPhase0()~loadPhase1()函数会按照下图所示顺序搜索动态库，并调用不同的函数来继续处理。
		 */
...
/**
 ...
 1.设置运行环境。
 这一步主要是设置运行参数、环境变量等。代码在开始的时候，将入参mainExecutableMH赋值给了sMainExecutableMachHeader，这是一个macho_header结构体，表示的是当前主程序的Mach-O头部信息，加载器依据Mach-O头部信息就可以解析整个Mach-O文件信息。接着调用setContext()设置上下文信息，包括一些回调函数、参数、标志信息等。设置的回调函数都是dyld模块自身实现的，如loadLibrary()函数实际调用的是libraryLocator()，负责加载动态库.
		...
		uintptr_t* startGlue)
{
	...
	/**
	 1.获取主程序的macho_header结构:
	 将入参mainExecutableMH赋值给了sMainExecutableMachHeader，这是一个macho_header结构体，表示的是当前主程序的Mach-O头部信息，加载器依据Mach-O头部信息就可以解析整个Mach-O文件信息。
	 */
	sMainExecutableMachHeader = mainExecutableMH;
	
	
	// 获取主程序的slide值: 根据滑块算出偏移地址(ASLR)
	...
	/**
	 2.设置上下文信息
	 接着调用setContext()设置上下文信息，包括一些回调函数、参数、标志信息等。
	 设置的回调函数都是dyld模块自身实现的，如loadLibrary()函数实际调用的是libraryLocator()，负责加载动态库。
	 */
	...
	/**
	 ...
	 configureProcessRestrictions()用来配置进程是否受限，代码逻辑比较简单，sEnvMode默认等于envNone（即受限模式），
	 如果设置了get_task_allow权限或者是内核开发时会设置成envAll，如果设置了uid和gid则立即变成受限模式
		...
		
	{
		/**
		 4.检测环境变量
			checkEnvironmentVariables()检测环境变量，如果sEnvMode为envNone就直接返回，
		 否则调用processDyldEnvironmentVariable()处理并设置环境变量
		 */
	...
	/**
	 1.检查共享缓存是否开启，iOS必须开启
	 这一步先调用checkSharedRegionDisable()检查共享缓存是否禁用。
	 该函数的iOS实现部分仅有一句注释，从注释我们可以推断iOS必须开启共享缓存才能正常工作
	 */
	...
	if ( gLinkContext.sharedRegionMode != ImageLoader::kDontUseSharedRegion ) {
		...
		/**
		 2.接下来调用mapSharedCache()加载共享缓存，
		 
		 而mapSharedCache()里面实则是调用了loadDyldCache()，从代码可以看出，共享缓存加载又分为三种情况：
...
reloadAllImages:
		...
		/**
		 这一步将主程序的Mach-O加载进内存，并实例化一个ImageLoader（ImageLoader是抽象类，其子类负责把Mach-O文件实例化为image）
		 
		 instantiateFromLoadedImage()首先调用isCompatibleMachO()检测Mach-O头部的magic、cputype、cpusubtype等相关属性，
		 
		 判断Mach-O文件的兼容性，如果兼容性满足，则调用ImageLoaderMachO::instantiateMainExecutable()实例化主程序的ImageLoader
		 
		 最后将image加入到sAllImages全局镜像列表，并将image映射到申请的内存中。
		 */
		
		// 详解在instantiateFromLoadedImage函数中
		...
		if	( sEnv.DYLD_INSERT_LIBRARIES != NULL ) { // 根据 DYLD_INSERT_LIBRARIES 环境变量来决定是否需要加载插入的动态库 .
...
#endif
		/// link()主要函数。 link函数主要做了循环加载依赖库,对主程序和依赖库做重定位rebase,符号绑定,弱绑定,注册GOF。
		link(sMainExecutable, sE nv.DYLD_BIND_AT_LAUNCH, true, ImageLoader::RPathChain(NULL, NULL), -1);
		...
		if ( sInsertedDylibCount > 0 ) {
			...
			for(unsigned int i=0; i < sInsertedDylibCount; ++i) {
				...
				/// sAllImages中保存的第一项是主程序的镜像，所以要从i+1的位置开始，取到的才是动态库的ImageLoader
				ImageLoader* image = sAllImages[i+1];
				
/**
		接下来循环调用每个镜像的registerInterposing()函数，
		该函数会遍历Mach-O的LC_SEGMENT_COMMAND加载命令，读取__DATA,__interpose，并将读取到的信息保存到fgInterposingTuples中
		
		接着调用applyInterposing()函数，内部经由doInterpose()虚函数进行替换操作，
		以ImageLoaderMachOCompressed::doInterpose()函数的实现为例：
			该函数内部调用了eachBind()与eachLazyBind()，具体处理函数是interposeAt()，
			该函数调用interposedAddress()在fgInterposingTuples中查找需要替换的符号地址，进行最终的符号地址替换
 */
		...
		/**
		 weakBind()首先通过getCoalescedImages()合并所有动态库的弱符号到一个列表里，
		 然后调用initializeCoalIterator()对需要绑定的弱符号进行排序，
		 接着调用incrementCoalIterator()读取dyld_info_command结构的weak_bind_off和weak_bind_size字段，
		 确定弱符号的数据偏移与大小，最终进行弱符号绑定
		 */
...
//	*******以上的所有步骤都是在加载Match-O,从initializeMainExecutable函数开始一步一步调用主程序代码。*******//
		...
		/**
		 ...
		 内部再调用processInitializers()
		 内部再调用recursiveInitialization()。
		 我们在recursiveInitialization()函数里找到了notifySingle()函数，
		 recursiveInitialization调用完dyld::notifySingle:后,会继续调用doInitialization函数,进入doInitialization函数
		 
		 在dyld::notifySingle:中找load_images时,却找不到。load_images是objc（runtime）注册的回调。
		 在这里runtime会对项目中的所有类结构进行初始化，然后调用所有的load方法
...
#endif
		{
			// find entry point for main executable
			 ...
			 // 第九步 查找入口点并返回
			/**
			 这一步调用主程序镜像的getEntryFromLC_MAIN()，从加载命令读取LC_MAIN入口，
			 如果没有LC_MAIN就调用getMain()读取LC_UNIXTHREAD，找到后就跳到入口点指定的地址并返回。
